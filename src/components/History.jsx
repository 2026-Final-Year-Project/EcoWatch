...
function RunCard({ run, darkMode }) {
  return <article className={`rounded-2xl border p-6 ${darkMode ? 'border-white/10 bg-white/5' : 'border-slate-100 bg-white'}`}>
...
export default function History() {
  ...
  const filteredRuns = useMemo(() => runs.filter(run => (resultFilter === 'all' || (resultFilter === 'detected' ? run.miningDetected : !run.miningDetected)) && (sourceFilter === 'all' || run.source === sourceFilter)).sort((left, right) => sortBy === 'oldest' ? new Date(left.analysedAt) - new Date(right.analysedAt) : new Date(right.analysedAt) - new Date(left.analysedAt)), [runs, resultFilter, sourceFilter, sortBy])
  ...
  return <div className={`min-h-screen font-sans ${darkMode ? 'bg-[#0f1a0a] text-white' : 'bg-[#f6f7f1] text-slate-900'}`}>
    <header className={`flex items-center justify-between border-b px-6 py-4 ${darkMode ? 'border-white/10 bg-[#111a09]' : 'border-black/5 bg-white'}`}><Link href="/" aria-label="Go to homepage"><Image src="/Area.png" alt="EcoWatch Logo" width={90} height={40} priority /></Link><nav className="flex gap-8 text-sm"><Link href="/monitor" className="text-slate-500 hover:text-slate-800">Live Map</Link><Link href="/report" className="text-slate-500 hover:text-slate-800">Reports</Link><Link href="/history" className="border-b-2 border-[#4a5e1a] pb-0.5 font-semibold text-[#4a5e1a]">History</Link></nav><button type="button" onClick={toggleTheme} aria-label="Toggle colour theme" className="text-lg">{darkMode ? <SunIcon /> : <MoonIcon />}</button></header>
    <main className="mx-auto max-w-7xl px-6 py-10"><div className="mb-8"><p className="text-xs font-mono uppercase tracking-[0.2em] text-[#4a5e1a]">Recorded model activity</p><h1 className="mt-2 text-4xl font-bold">Analysis history</h1><p className={`mt-3 text-sm ${darkMode ? 'text-white/65' : 'text-slate-600'}`}>Every completed satellite-model analysis from the live map and community reporting flow is recorded here.</p>{apiError && <p role="alert" className="mt-3 text-sm text-red-600">{apiError}</p>}</div>
      ...
      <section className={`mb-8 rounded-2xl border p-5 ${panel}`}><div className="grid gap-4 md:grid-cols-[1fr_1fr_1fr_auto] md:items-end"><label className="text-xs font-mono uppercase tracking-widest text-slate-400">Result<select value={resultFilter} onChange={event => setResultFilter(event.target.value)} className={`mt-2 block w-full rounded-lg border px-3 py-2 text-sm normal-case tracking-normal ${input}`}><option value="all">All results</option><option value="detected">Mining signal detected</option><option value="none">No mining detected</option></select></label><label className="text-xs font-mono uppercase tracking-widest text-slate-400">Source<select value={sourceFilter} onChange={event => setSourceFilter(event.target.value)} className={`mt-2 block w-full rounded-lg border px-3 py-2 text-sm normal-case tracking-normal ${input}`}><option value="all">All sources</option><option value="map">Live map</option><option value="community-report">Community report</option></select></label><label className="text-xs font-mono uppercase tracking-widest text-slate-400">Order<select value={sortBy} onChange={event => setSortBy(event.target.value)} className={`mt-2 block w-full rounded-lg border px-3 py-2 text-sm normal-case tracking-normal ${input}`}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label><div className="flex rounded-lg border border-slate-200 p-1 text-sm"><button onClick={() => setViewMode('timeline')} className={`rounded px-3 py-2 ${viewMode === 'timeline' ? 'bg-[#4a5e1a] text-white' : ''}`}>Timeline</button><button onClick={() => setViewMode('list')} className={`rounded px-3 py-2 ${viewMode === 'list' ? 'bg-[#4a5e1a] text-white' : ''}`}>List</button></div></div></section>
      {loading ? <div className={`rounded-2xl border p-8 text-sm text-slate-500 ${panel}`}>Loading recorded analyses...</div> : filteredRuns.length === 0 ? <div className={`rounded-2xl border p-8 text-center text-sm text-slate-500 ${panel}`}>No completed model analyses match these filters. Run a satellite check from the live map to start building history.</div> : viewMode === 'timeline' ? <section className="space-y-4">{filteredRuns.map(run => <RunCard key={run.id} run={run} darkMode={darkMode} />)}</section> : <RunTable runs={filteredRuns} darkMode={darkMode} />}
      <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-slate-500">Showing {filteredRuns.length} recorded model run{filteredRuns.length === 1 ? '' : 's'}.</p><div className="flex gap-3"><button type="button" disabled={loading} onClick={() => exportPdf('EcoWatch Model Analysis Export', createHistoryAnalyticsFilename)} className={`rounded-lg border px-4 py-2 text-sm font-medium disabled:opacity-50 ${darkMode ? 'border-white/20' : 'border-slate-200'}`}>Export analytics</button><button type="button" disabled={loading} onClick={() => exportPdf('EcoWatch Historical Model Report', createHistoricalReportFilename)} className="rounded-lg bg-[#4a5e1a] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Download historical report</button></div></div>
    </main>
  </div>
}
